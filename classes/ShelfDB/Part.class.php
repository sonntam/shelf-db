  ...
  class Part {
    ...
    private function GetSegmentByTypeId( string $type, $id, int $offset, int $limit, $sortcol, $sortorder, $recursive, $search = null) {
      ...
      $query = "SELECT p.*, p.id, p.id_category, p.name AS name, p.instock AS instock, p.mininstock AS mininstock, "
        ...
        ."ANY_VALUE(COALESCE(pr.price, '0')) AS price, "
        ."ANY_VALUE(COALESCE(mpf.pict_fname, fpr.pict_fname, 'default.png')) AS mainPicFileName, "
        ."ANY_VALUE(COALESCE(mtpf.pict_fname, tfpr.pict_fname)) AS mainThumbnailPicFileName, "
        ."ANY_VALUE(COALESCE(fpr.pict_fname,'default.png')) AS f_pict_fname, "
        ."ANY_VALUE(COALESCE(sup.pict_fname,'default.png')) AS su_pict_fname, "
        ."ANY_VALUE(fpr.pict_width) as f_pict_width, "
